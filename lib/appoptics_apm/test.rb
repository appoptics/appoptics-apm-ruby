    ...
    class << self
      ...
      def set_postgresql_env
        if ENV.key?('TRAVIS_PSQL_PASS')
          ...
          # ENV['DATABASE_URL'] = "postgresql://#{ENV['POSTGRES_USER']}:#{ENV['DOCKER_PSQL_PASS']}@#{ENV['PSQL_HOST']}:5432/travis_ci_test"
          ENV['DATABASE_URL'] = "postgresql://postgres:#{ENV['DOCKER_PSQL_PASS']}@#{ENV['PSQL_HOST']}:5432/travis_ci_test"
