  ...
  describe 'GRPC' do
    ...
    describe 'UNARY' do
      ...
      it 'sends metrics from the server for unary' do
        Libappoptics_apm::Span.expects(:createSpan).with('AddressService.unary', nil, is_a(Integer))
        @stub.unary(@address_msg)
        sleep 0.5
      end
    end
    ...
    describe 'CLIENT_STREAMING' do
      ...
      it 'sends metrics from the server for client_streaming' do
        Libappoptics_apm::Span.expects(:createSpan).with('AddressService.client_stream', nil, is_a(Integer))
    ...
    describe 'SERVER_STREAMING return Enumerator' do
      ...
      it 'sends metrics from the server for server_streaming with enumerator' do
        Libappoptics_apm::Span.expects(:createSpan).with('AddressService.server_stream', nil, is_a(Integer))
    ...
    describe 'BIDI_STREAMING return Enumerator' do
      ...
      it 'sends metrics from the server for bidi_streaming with enumerator' do
        Libappoptics_apm::Span.expects(:createSpan).with('AddressService.bidi_stream', nil, is_a(Integer))
        response = @stub.bidi_stream([@null_msg, @null_msg])
        response.each { |_| }
        sleep 0.5
      end
